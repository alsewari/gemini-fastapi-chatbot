...
# Models shown in the user interface.
# These are current Gemini Developer API models with a documented free tier.
# Keep this allowlist on the server so users cannot submit arbitrary model names.
MODEL_OPTIONS: dict[str, dict[str, str | bool]] = {
    "gemini-3.6-flash": {
        "label": "Gemini 3.6 Flash",
        "description": "Recommended for general chat, coding, reasoning, and multimodal files.",
        "recommended": True,
    },
    "gemini-3.5-flash-lite": {
        "label": "Gemini 3.5 Flash-Lite",
        "description": "Fast and economical for simple chat, summaries, and high-volume use.",
        "recommended": True,
    },
    "gemini-3.5-flash": {
        "label": "Gemini 3.5 Flash",
        "description": "Strong general-purpose model for coding and more complex tasks.",
        "recommended": False,
    },
    "gemini-3.1-flash-lite": {
        "label": "Gemini 3.1 Flash-Lite",
        "description": "Efficient model for lightweight, high-volume tasks.",
        "recommended": False,
    },
    "gemini-2.5-flash": {
        "label": "Gemini 2.5 Flash",
        "description": "Stable multimodal model with a good balance of quality and speed.",
        "recommended": False,
    },
    "gemini-2.5-flash-lite": {
        "label": "Gemini 2.5 Flash-Lite",
        "description": "Lightweight model for quick and simple requests.",
        "recommended": False,
    },
    "gemini-2.5-pro": {
        "label": "Gemini 2.5 Pro",
        "description": "Useful for complex reasoning, analysis, and coding.",
        "recommended": False,
    },
}
...
class ChatRequest(BaseModel):
    ...
    model: str = Field(
        ...
        description="Choose a model returned by GET /api/models.",
    )
...
def validate_model(model: str | None) -> str:
    """Validate a model selected by the user against the server allowlist."""
    selected_model = (model or DEFAULT_MODEL).strip()

    if selected_model not in MODEL_OPTIONS:
        allowed_models = ", ".join(MODEL_OPTIONS)
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported model. Choose one of: {allowed_models}",
        )
...
async def create_gemini_response(
    *,
    ...
    client = get_gemini_client()
    model = validate_model(selected_model)
...
def health() -> dict[str, str | int]:
    return {
        ...
        "available_models": len(MODEL_OPTIONS),
        "max_files": MAX_FILES,
...
def list_models() -> dict[str, Any]:
    """Return the models approved for selection in the chatbot."""
    return {
        "default_model": DEFAULT_MODEL,
        "models": [
            {
        ...
        "notice": (
            "Free-tier use is quota-limited and availability can vary by "
            "Google project, account, region, and future pricing changes."
        ),
    }
        ...
        <form id="chatForm">
            <div class="composer">
                <div class="model-row">
                    ...
                    <select id="modelSelect" aria-label="Select an LLM model"></select>
                    <span id="modelDescription">Loading available models...</span>
                </div>
    ...
    <script>
        ...
        function updateModelDescription() {{
            const selected = availableModels.find(
                item => item.id === modelSelect.value
            );
            ...
            const recommendation = selected.recommended
                ? " Recommended."
                : "";

            modelDescription.textContent =
                `${{selected.description}} Free-tier eligible.${{recommendation}}`;
        }}
        ...
        async function loadModels() {{
            try {{
                ...
                availableModels.forEach(item => {{
                    ...
                    option.selected = item.id === data.default_model;
                ...
                modelDescription.textContent =
                    "Could not load the model catalogue. Using the default model.";
            }}
        }}

        modelSelect.addEventListener("change", () => {{
            // An interaction ID belongs to the model that created it.
            // Start a new conversation whenever the user changes model.
            previousInteractionId = null;
            updateModelDescription();
            messages.innerHTML =
                `<div class="message bot">Model changed to ${{modelSelect.value}}. A new conversation has started.</div>`;
            input.focus();
        }});
